...
// void write_cr4(unsigned long);
#ifndef read_cr4
#define read_cr4 native_read_cr4_custom
static inline unsigned long native_read_cr4_custom(void) {
  unsigned long val;
...
#endif
#ifndef write_cr4
#define write_cr4 native_write_cr4_custom
static inline void native_write_cr4_custom(unsigned long val) {
...
int pmc_init_module(void) {
  ...
  if ((mod_major = register_chrdev(NVMEMUL_MAJOR, module_name, &pmc_fops)) ==
      -EBUSY) {
...
static long pmc_ioctl_setcounter(struct file *f, unsigned int cmd,
                                 unsigned long arg) {
  ioctl_query_setcounter_t q;

  if (copy_from_user(&q, (ioctl_query_setcounter_t *)arg,
                     sizeof(ioctl_query_setcounter_t))) {
    return -EFAULT;
  }
...
static long pmc_ioctl_setpci(struct file *f, unsigned int cmd,
                             unsigned long arg) {
  ioctl_query_setgetpci_t q;
  struct pci_bus *bus = NULL;

  if (copy_from_user(&q, (ioctl_query_setgetpci_t *)arg,
                     sizeof(ioctl_query_setgetpci_t))) {
    return -EFAULT;
  }

  while ((bus = pci_find_next_bus(bus))) {
    if (q.bus_id == bus->number) {
      pci_bus_write_config_word(bus, PCI_DEVFN(q.device_id, q.function_id),
                                q.offset, (u16)q.val);
      printk(
          KERN_INFO
          "%s: setpci bus_id=0x%x device_id=0x%x, function_id=0x%x, val=0x%x\n",
...
static long pmc_ioctl_getpci(struct file *f, unsigned int cmd,
                             unsigned long arg) {
  ioctl_query_setgetpci_t q;
  struct pci_bus *bus = NULL;
  ...
  while ((bus = pci_find_next_bus(bus))) {
    if (q.bus_id == bus->number) {
      unsigned int val = 0;
      pci_bus_read_config_word(bus, PCI_DEVFN(q.device_id, q.function_id),
                               q.offset, (u16 *)&val);
      printk(KERN_INFO "%s: getpci bus_id 0x%x device_id 0x%x, function_id "
                       "0x%x, offset 0x%x, val 0x%x\n",
             module_name, q.bus_id, q.device_id, q.function_id, q.offset, val);
      q.val = val;
      if (copy_to_user((ioctl_query_setgetpci_t *)arg, &q,
                       sizeof(ioctl_query_setgetpci_t))) {
        return -EFAULT;
      }
      return 0;
